...
const lmCharacter = {
	...
	skill: {
		...
		old_olqiangang: {
			...
			enable: "phaseUse",
			filter(event, player) {
				return !player.hasSkill("olrumo", null, false, false) && player.hasSkill("old_oltianen", null, false, false);
			},
			skillAnimation: true,
			animationColor: "wood",
			manualConfirm: true,
		...
		old_olguanbian: {
			...
			trigger: {
				...
				player: ["enterGame", "olxiongniAfter", "old_olfengshangAfter"],
			},
			filter(event, player, name) {
			...
			forced: true,
			async content(event, trigger, player) {
				if (event.triggername == "roundStart" || ["olxiongni", "old_olfengshang"].includes(trigger.name)) await player.removeSkills(event.name);
		...
		old_clanlieshi: {
			...
			chooseButton: {
				dialog(event, player) {
					...
					dialog.add([lib.skill.old_clanlieshi.choices.slice(), "textbutton"]);
					return dialog;
				},
				filter(button, player) {
				...
				backup(links) {
					var next = get.copy(lib.skill["old_clanlieshi_backupx"]);
					next.choice = links[0];
					return next;
				},
				prompt(links) {
			...
			subSkill: {
				...
				backupx: {
					...
					multitarget: true,
					async content(event, trigger, player) {
						const choice = lib.skill.old_clanlieshi_backup.choice;
						if (choice == "damage") {
						...
						const list = [];
						let choiceList = lib.skill.old_clanlieshi.choices.slice();
		...
		old_clanxumin: {
			...
			async precontent(event, trigger, player) {
				player.logSkill("old_clanxumin");
				player.awakenSkill("old_clanxumin");
			},
			async content(event, trigger, player) {
				const result = await player
					.chooseTarget({
						prompt: "选择【五谷丰登】的目标（可多选）",
				...
				const card = { name: "wugu", isVirtual: true, suit: "", number: 0 };
				await player.useCard(card, targets, { skipTargetCheck: true });
			},
			ai: {
				order: 7,
				result: { target: 1 },
		...
		old_olsiqi: {
				...
				while (cards.length) {
					...
					if (result2.bool) {
						...
						game.broadcastAll(card => {
							...
							lib.skill.old_olsiqi_backup.card = card;
						}, card);
			...
			subSkill: {
				backup: {
					...
					log: false,
					async precontent(event, trigger, player) {
						const { card } = get.info("old_olsiqi_backup");
		...
		old_aichen: {
			...
			async content(event, trigger, player) {
				...
				switch (index) {
					case 0: // 回复
						const targets0 = await player
							.chooseTarget("选择一名受伤的角色", (card, player, target) => target.isDamaged())
							.set("ai", target => get.recoverEffect(target, player, player))
							.forResult();
						target = targets0.targets[0];
						break;
					case 1: // 失去体力
						const targets1 = await player
							.chooseTarget("选择一名角色", () => true)
							.set("ai", target => get.effect(target, { name: "losehp" }, player, player))
							.forResult();
						target = targets1.targets[0];
						break;
					case 2: // 弃牌
						const targets2 = await player
							.chooseTarget("选择一名有手牌或装备的角色", (card, player, target) => target.countCards("he") > 0)
							.set("ai", target => get.effect(target, { name: "guohe_copy2" }, player, player) * Math.sqrt(Math.min(2, target.countCards("he"))))
							.forResult();
						target = targets2.targets[0];
						break;
					case 3: // 摸牌分配（目标可以是任意角色）
						const targets3 = await player
							.chooseTarget("选择一名角色", () => true)
							.set("ai", target => 2 * get.effect(target, { name: "draw" }, player, player))
							.forResult();
		...
		old_xionghuo: {
			...
			subSkill: {
				...
				disable: {
					mod: {
						playerEnabled(card, player, target) {
							if (card.name == "sha" && player.getStorage("old_xionghuo_disable").includes(target)) {
								return false;
							}
						},
					},
		...
		old_dcsbzhenyu: {
					...
					return {
						...
						async precontent(event, trigger, player) {
							...
							const { targets } = await player
								...
								.forResult();
							if (targets?.length) {
								await player.logSkill("old_dcsbzhenyu");
			...
			subSkill: {
				...
				disable: {
					...
					charlotte: true,
					init(player, skill) {
						player.disableSkill(skill, "old_dcsbzhenyu");
					},
					onremove(player, skill) {
			...
			hiddenCard(player, name) {
				return get.type(name) == "basic" && lib.inpile.includes(name) && game.hasPlayer(current => !current.isLinked());
			},
			ai: {
				respondSha: true,
				respondShan: true,
		...
		old_dcjiangxian: {
			...
			async content(event, trigger, player) {
				...
				player
					...
					.filter((evt, player) => {
						return evt == evtx && ["dcchaozhen", "old_dclianjie"].some(skill => player.hasSkill(skill, null, null, false));
					})
					.step(async () => {
						const { bool, links } = await player
							.chooseButton(
		...
		old_mbqianlong: {
			...
			beginMarkCount: 20,
			maxMarkCount: 99,
			derivation: ["mbcmqingzheng", "mbcmjiushi", "old_mbcmfangzhu", "old_mbjuejin"],
			addMark(player, num) {
		...
		old_mbbojian: {
			...
			filter(event, player) {
				_status.old_mbbojian ??= {};
				_status.old_mbbojian[player.playerid] ??= [0, 0];
			...
			subSkill: {
				record: {
					init(player, skill) {
						_status.old_mbbojian ??= {};
						_status.old_mbbojian[player.playerid] ??= [0, 0];
						player.addTip(skill, `${get.translation(skill)} 上次 ${_status.old_mbbojian[player.playerid][0]}/${_status.old_mbbojian[player.playerid][1]}`);
						player.markSkill(skill);
					},
					onremove(player, skill) {
						player.removeTip(skill);
					...
					async content(event, trigger, player) {
						...
						player.addTip(event.name, `${get.translation(event.name)} 上次 ${_status.old_mbbojian[player.playerid][0]}/${_status.old_mbbojian[player.playerid][1]}`);
				...
				global: {
					mod: {
						cardEnabled(card, player) {
							...
							if (card.name == "tao" && source?.isIn() && source != player && source.hasSkill("old_sbwansha") && !player.isDying()) {
								return false;
							}
						},
						cardSavable(card, player) {
			...
			subSkill: {
				add: {
					...
					filter(event, player) {
						...
						return ["old_sbwansha", "sbweimu"].some(skill => player.hasSkill(skill, null, null, false) && !player.storage[skill]);
					},
					async cost(event, trigger, player) {
						const choices = [],
							list = ["old_sbwansha", "sbweimu"];
					...
					async content(event, trigger, player) {
						const list = ["old_sbwansha", "sbweimu"],
							index = ["选项一", "选项二"].indexOf(event.cost_data);
						player.storage[list[index]] = true;
					},
				},
			},
			derivation: ["old_sbwansha_rewrite", "sbweimu_rewrite"],
		},
	...
	translate: {
		...
		old_olguanbian: "观变",
		old_olguanbian_info: `锁定技。①游戏开始时，你的手牌上限、其他角色计算与你的距离、你计算与其他角色的距离+X（X为游戏人数）。②第二轮游戏开始时，或当你发动${get.poptip("olxiongni")}或${get.poptip("old_olfengshang")}后，你失去〖观变〗。`,
		old_olfengshang: "封赏",
		...
		old_olqianxi: "潜袭",
		old_olqianxi_info: "准备阶段，你可以摸一张牌并展示一张牌。若如此做，距离为1的其他角色本回合不能使用或打出与此牌颜色相同的手牌，你本回合使用此牌造成的伤害+1。",
		...
		//门阀士族
		old_clan_xuncai: "旧族荀采",
		old_clan_xuncai_prefix: "旧|族",
		old_clanlieshi: "烈誓",
		old_clanlieshi_info: "出牌阶段，你可以执行其中一项：『受到1点火焰伤害并废除判定区；弃置手牌中的所有【闪】；弃置手牌中的所有【杀】』，然后你令一名其他角色选择执行另一项（不能选择无法执行的选项）。",
		...
		old_clanxumin: "恤民",
		old_clanxumin_info: "宗族技，限定技，出牌阶段，你可以视为对任意名其他角色使用一张【五谷丰登】。",
		...
		old_dclinghui: "灵慧",
		old_dclinghui_info: "一名角色的结束阶段，若当前回合角色为你或本回合有角色进入过濒死状态，则你可以观看牌堆顶的三张牌并使用其中一张牌，然后获得剩余的牌。",
		...
		old_xurong_prefix: "旧",
		old_xionghuo: "凶镬",
		old_xionghuo_info: "游戏开始时，你获得3个“暴戾”标记。出牌阶段，你可以交给一名其他角色一个“暴戾”标记。当你对有“暴戾”标记的其他角色造成伤害时，此伤害+1。有“暴戾”标记的其他角色的出牌阶段开始时，其移去所有“暴戾”标记并随机执行一项：1.受到1点火焰伤害且本回合不能对你使用【杀】；2.失去1点体力且本回合手牌上限-1；3.你随机获得其两张牌。",
		old_shajue: "杀绝",
		...
		old_juejin: "绝烬",
		old_juejin_info: `${get.poptip("rule_chihengji")}。限定技，出牌阶段或当你进入濒死状态时，你可以选择X项执行：①回复X点体力；②摸X张牌；③手牌上限+X；④对一名其他角色造成X点火焰伤害（X为你脱离濒死状态的次数，至多为4，至少为1）。然后你可以发动一次${get.poptip("old_huitian")}。`,
		old_huitian: "回天",
		...
		oldx_juejin: "绝烬",
		oldx_juejin_info: `${get.poptip("rule_chihengji")}。限定技，出牌阶段或当你进入濒死状态时，你可以选择X项执行：①回复X点体力；②摸X张牌；③体力上限+X；④对一名其他角色造成X点雷电伤害（X为你脱离濒死状态的次数，至多为4，至少为1）。然后你可以发动一次${get.poptip("old_huitian")}。若你未使用此技能执行过选项，则你进入濒死状态后减一点体力上限并将体力回复至一点。`,
		oldx_xingzhen: "星阵",
		...
		old_mbqianlong: "潜龙",
		old_mbqianlong_info: `${get.poptip("rule_chihengji")}。①游戏开始时，你获得20枚“道心”标记。②当你得到牌后/受到1点伤害后/造成1点伤害后，你获得5/10/15枚“道心”（上限为99枚）。③若你的“道心”数不小于25/50/75/99，你视为拥有${get.poptip("mbcmqingzheng")}/${get.poptip("mbcmjiushi")}/${get.poptip("old_mbcmfangzhu")}/${get.poptip("old_mbjuejin")}。`,
		old_mbcmfangzhu: "放逐",
		...
		old_mbweitong: "卫统",
		old_mbweitong_info: `${get.poptip("rule_chihengji")}，主公技。游戏开始时，若你有${get.poptip("old_mbqianlong")}且场上有其他魏势力角色，则你获得x枚“道心”标记（x为其他魏势力角色数*20）。`,

		old_mb_cuilingyi: "旧手杀崔令仪",
		old_mb_cuilingyi_ab: "旧手杀崔芙",
		old_mb_cuilingyi_prefix: "旧|手杀",
		old_mbcaiqiu: "裁裘",
		old_mbcaiqiu_info: "①每轮开始时，你观看牌堆顶X张牌（X为游戏人数），然后可以获得其中任意张牌。②每轮限一次，其他角色使用牌结算结束后，若你本轮因〖裁裘〗获得过同名牌，你失去1点体力。",
		old_mbxishang: "袭裳",
		...
		old_hefeidangshi: "荡势",
		old_hefeidangshi_info: "当你使用伤害牌结算结束后，你可令一名其他目标角色选择一项：1.对你使用一张非转化、非虚拟且与此牌牌名相同的伤害牌；2.弃置X张牌（X为所有角色本轮选择此项的次数且至少为1）；3.受到你造成的1点伤害。若其选择了本阶段未被执行过的选项，你摸一张牌且本阶段出【杀】次数+1。",
		...
		old_sbyiji: "遗计",
		old_sbyiji_info: "当你受到1点伤害后或你每轮首次进入濒死状态时，你可以摸三张牌，然后你可以将至多等量张牌交给任意名其他角色。",
